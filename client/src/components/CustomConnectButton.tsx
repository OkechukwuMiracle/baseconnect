...
export default function CustomConnectButton() {
  ...
  // Track which wallet address has been authenticated
  ...
  // Backend wallet authentication flow - only trigger when not on signup/login pages
  useEffect(() => {
    // Don't auto-authenticate on signup/login pages - those pages handle their own flow
    ...
    if (authenticatedAddress.current === lower || authInProgress.current) {
      return;
    }
    ...
    const storedToken = localStorage.getItem("token");
    if (storedToken) {
      authenticatedAddress.current = lower;
      return;
    }
  ...
  // Reset authenticated address when wallet disconnects
  useEffect(() => {
    if (!isConnected) {
      authenticatedAddress.current = null;
      authInProgress.current = false;
