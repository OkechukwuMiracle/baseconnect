...
export default function Login(){
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // fetch /me
      const meRes = await fetch(`${import.meta.env.VITE_API_URL}/api/auth/me`, { headers: { Authorization: `Bearer ${data.token}` }});
      const me = await meRes.json();
      await refresh();
      if (!me.profileCompleted) {
        toast({ title: 'Welcome back', description: 'Continue onboarding to complete your profile.' });
        navigate('/onboarding');
      } else {
        const next = me.role === 'creator' ? '/dashboard/creator' : '/dashboard/contributor';
      ...
      <div className="pt-24 pb-12 px-4 container mx-auto max-w-md">
        <Card>
          ...
          <CardContent>
            ...
            <p className="mt-4 text-sm text-muted-foreground">No account? <a className="underline" href="/signup">Sign up</a></p>
          </CardContent>
        </Card>
      </div>
    </div>
