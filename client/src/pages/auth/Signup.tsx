...
export default function Signup() {
  ...
  // Track if wallet signup was initiated by this component
  const walletSignupInitiated = useRef(false);
  const hasProcessedAuth = useRef(false);
  // Track if regular signup is in progress to prevent unmounting issues
  const signupInProgress = useRef(false);

  // Disconnect any existing wallet connection when component mounts
  useEffect(() => {
    if (isConnected && !walletSignupInitiated.current) {
      console.log("Disconnecting existing wallet connection on signup page mount");
      disconnect();
    }
  ...
  const handleWalletSignup = async () => {
    ...
    if (isConnected) {
      ...
      setTimeout(() => {
        if (openConnectModal) {
          walletSignupInitiated.current = true;
  ...
  // Handle wallet connection ONLY when initiated from this signup page
  useEffect(() => {
    // Don't process wallet auth if regular signup is in progress
    if (signupInProgress.current) {
      return;
    }

    // Only proceed if:
    // 1. User clicked wallet signup button (walletSignupInitiated is true)
    // 2. Wallet is now connected
    // 3. We haven't processed this connection yet
    if (!walletSignupInitiated.current || !isConnected || !connectedAddress || hasProcessedAuth.current) {
      return;
    }
    ...
    const performAuth = async () => {
      ...
      try {
        ...
        const isNewUser = !user.role || user.role === null;
        ...
        hasProcessedAuth.current = false;
        walletSignupInitiated.current = false;
        if (mounted) {
          setWalletLoading(false);
        }
      }
  ...
  }, [isConnected, connectedAddress, authenticateWithWallet, navigate, toast]);
            ...
            <CardContent>
              <form onSubmit={submit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="Jane"
                      className={`placeholder:text-gray-300 placeholder:font-light ${errors.firstName ? 'border-destructive' : ''}`}
                      disabled={submitting}
                      required
                    />
                    {errors.firstName && (
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="Doe"
                      className={`placeholder:text-gray-300 placeholder:font-light ${errors.lastName ? 'border-destructive' : ''}`}
                      disabled={submitting}
                      required
                    />
                    {errors.lastName && (
                ...
                <div>
                  ...
                  <div className="relative">
                    <Input
                      ...
                      placeholder="Confirm your password"
                      className={`placeholder:text-gray-300 pr-10 placeholder:font-light ${errors.confirmPassword ? 'border-destructive' : ''}`}
                      disabled={submitting}
                      required
                    />
                    <button
                ...
                <div className="flex items-start">
                  ...
                  <span className="ml-2 text-[12px] text-muted-foreground">
                    Yes, I understand and agree to the
                    <a href="/terms" className="underline text-primary hover:text-blue-600">
                      {" "}Terms of Service{" "}
                    </a>
                    and
                    <a href="/privacy" className="underline text-primary hover:text-blue-600">
                      {" "}Privacy Policy
                    </a>
                  </span>
                </div>
              ...
              <div className="space-y-3 mt-7">
                <button
                  ...
                  onClick={handleGoogleSignup}
                  disabled={submitting}
                  className="flex gap-2 items-center border-2 border-[#B4D3FF] p-2.5 rounded-xl w-full justify-center text-[14px] font-semibold hover:text-white hover:bg-gradient-hero transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <FcGoogle className="w-5 h-5" />
                  Sign up with Google
                </button>
                <button
                  ...
                  className="flex gap-2 items-center border-2 border-dashed border-primary/40 p-2.5 rounded-xl w-full justify-center text-[14px] font-semibold hover:text-white hover:bg-gradient-hero transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {walletLoading ? (
                    <>
                      <Loader2 className="h-4 w-4 animate-spin" />
